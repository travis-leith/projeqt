...
.dataaccess.camel:{$[11h~type x;@[;0;upper]each string x;@[string x;0;upper]]};
...
.dataaccess.absagg:{enlist[`$x,y]!enlist(value x;`$x,y)};

// functions to calculate avg, cov and var in mapaggregate dictionary
.dataaccess.scx:{(sum;`$"count",x)};
.dataaccess.avgf:{(%;(sum;`$"sum",x);.dataaccess.scx y)};
.dataaccess.swsum:{(sum;`$"wsum",x,y)};
.dataaccess.covf:{(-;(%;.dataaccess.swsum[x;y];.dataaccess.scx x);(*;.dataaccess.avgf[x;x];.dataaccess.avgf[y;x]))};
.dataaccess.varf:{(-;(%;(sum;`$"sumsq",y);.dataaccess.scx x);(xexp;.dataaccess.avgf[y;x];2))};
...
.dataaccess.processres:{[options;res]
...
.dataaccess.mapreduceres:{[options;res]
    ...
    res:?[res;();gr;raze{.dataaccess.mapaggregate[x 0;.dataaccess.camel x 1]}'[aggs]];
...
.dataaccess.razeresults:{[options;res]
    res:raze res;
    .dataaccess.processres[options;res]
    };

// join results together if from multiple processes
.dataaccess.autojoin:{[options]
    ...
    :$[not options`mapreduce;.dataaccess.razeresults[options;];.dataaccess.mapreduceres[options;]];
    };


// Dynamic routing finds all processes with relevant data 
.dataaccess.attributesrouting:{[options;procdict]
...
.dataaccess.mapreduce:{[aggs;gr]
    ...
    // then error
    if[not all aggs[;0]in key .dataaccess.aggadjust;
...
.dataaccess.adjustqueries:{[options;part]
    ...
    if[(1<count dates)&`aggregations in key options;
        if[all key[o:options`aggregations]in key .dataaccess.aggadjust;
            aggs:.dataaccess.mapreduce[o;$[`grouping in key options;options`grouping;`]];
...
.dataaccess.getdata:{[o]
    ...
    options:default,o;
    if[`ordering in key o;options[`ordering]: .dataaccess.go each options`ordering];
    o:.dataaccess.adjustqueries[o;.dataaccess.partdict o];
    options[`mapreduce]:0b;
    gr:$[`grouping in key options;options`grouping;`];
    if[`aggregations in key options;
        if[all key[options`aggregations]in key .dataaccess.aggadjust;
    ...
    :$[.gw.call .z.w;
        // if sync
        .gw.syncexecjt[(`getdata;o);options[`procs];.dataaccess.autojoin[options];options[`timeout]];
        // if async
        .gw.asyncexecjpt[(`getdata;o);options[`procs];.dataaccess.autojoin[options];options[`postback];options[`timeout]]];
    };
