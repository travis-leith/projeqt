...
.dataaccess.filterskey:{[filtersstrings]
...
.dataaccess.jsontodict:{
    ...
    if [`filters in k;dict[`filters]:.dataaccess.filterskey dict`filters];
    //output
    :dict};
...
.dataaccess.qrest:{
    ...
    // Run the function 
    :.dataaccess.getdata .dataaccess.jsontodict x};


.dataaccess.quotefinder:{y[2#where y>x]};
