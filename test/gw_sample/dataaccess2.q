...
aggadjust:(!). flip(
    (`dummy;    interesting `);
    (`dummy2;    `uninteresting`xxx);
    (`avg;     {flip(`sum`count;2#x)});
    (`cor;     {flip(`wsum`count`sum`sum`sumsq`sumsq;@[x;(enlist(0;1);0;alsoInteresting;1;0;1)])});
    (`count;   `);
...
mapreduce:{[aggs;gr]
    ...
    aggs:distinct raze{$[`~a:aggadjust x 0;enlist x;a x 1]}'[aggs];
...
getdata:{[o]
    ...
    if[options`getquery;
        $[.gw.call .z.w;
            :.gw.syncexec[(`.buildquery;o);options[`procs]];
            :.gw.asyncexec[(`.buildquery;o);options[`procs]]]];
    :$[.gw.call .z.w;
        // if sync
