...
.gw.buildcross:{(cross/){flip (enlist y)#x}[x] each key x};
...
.gw.getserverscross:{[req;att;besteffort]

 if[0=count req; :([]serverid:enlist key att)];

 s:.gw.getserversinitial[req;att];

 /- build the cross product of requirements
 reqcross:.gw.buildcross[req];

 /- calculate the cross product of data contributed by each source
 /- and drop it from the list of stuff that is required
 util:flip `remaining`found!flip ({[x;y;z] (y[0] except found; y[0] inter found:$[0=count y[0];y[0];.gw.buildcross x@'where each z])}[req]\)[(reqcross;());value s];
...
.gw.getserveridstype:{[att;typ]
  ...
  res:$[attype=`independent;.gw.getserversindependent[att;servers;besteffort];
  .gw.getserverscross[att;servers;besteffort]];
...
.gw.getserverids:{[att]
  ...
  serverids:$[`servertype in key att;
    raze .gw.getserveridstype[delete servertype from att] each (),att`servertype;
    .gw.getserveridstype[att;`all]];
